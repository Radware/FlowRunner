...
//
// Ported from the wave2 engine spike (spikes/react-flow/src/flowModelAdapter.js).
// The Drawflow adapter (flowVisualizer.js) does this same walk imperatively with
// editor.addNode / addConnection; here it is declarative.
//
...
const ROLE_COLORS = {
    // Expressed as CSS custom-property references so the island consumes the
    // Wave-1 OKLCH tokens from styles.css instead of hardcoded hexes.
...
// _getOutputRole in flowVisualizer.js.
...
export function flowModelToReactFlow(flowModel, positions = {}) {
    ...
    // Track the previous sibling per branch so we can draw the sequential "main"
    // edges (step N -> step N+1) exactly like the linear runner order.
