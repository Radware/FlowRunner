...
## Unreleased — "Tidy Up" auto-layout button + on-node error badges (UX Wave 2, lane canvas)
- **Decision:** wired the Wave 1 `autoLayout.js` adapter into the flow map via a **"Tidy Up"** toolbar button. The button runs `computeLayout(steps, { nodeSizes, direction:'DOWN' })` (real rendered node sizes so layout respects card dimensions) and applies the `{x,y}` map through a new `FlowVisualizer.applyLayout(positions, { animate, onlyStepIds })`. **Why animate via FLIP:** the DoD requires a ~200ms ease-out that touches **transform/opacity only** — so `applyLayout` commits the final `left/top` immediately (keeping Drawflow geometry + connection routing correct) and animates the *visual delta* with a transient `.tidy-relayout` transform transition, never animating `left/top` (no layout thrash, honors `prefers-reduced-motion`).
- **Preserve manual placement:** Alt-click Tidy = **Tidy-selection** (`onlyStepIds` = the selected step + its rendered descendants); plain click = Tidy-all. Unselected nodes keep their positions.
- **Single-key undo:** `applyLayout` snapshots pre-apply positions; **Cmd/Ctrl+Z** in graph view reverts in one step (`undoLayout`/`canUndoLayout`). **No auto-jump:** opening an old saved layout still never pans (render() applies saved positions without `focusNode`); Tidy itself doesn't pan the viewport either.
- **On-node error badges:** per-step run failures now surface **on the node** — a red border/pip (`--run-error` token) plus a clickable `!` badge whose title/click reveals the message. State is recorded in a `nodeErrors` map (survives re-render, re-applied in `render()`), fed automatically from the run-result data the visualizer already receives (`highlightNode(id,'error')` and `updateNodeRuntimeInfo` with `status:'error'`), and reset by `clearHighlights()`/`clearNodeErrors()`. Added **"Next Error"** toolbar button + `jumpToNextError()` that focuses the next failed step cyclically in document order.
- **Contract:** added `applyLayout`/`undoLayout`/`canUndoLayout`/`jumpToNextError` to the FlowVisualizer public surface — documented in `docs/visualizer-contract.md` and pinned in `__tests__/visualizerContract.test.js` (with FakeVisualizer implementations). No frozen `.flow.json` field touched (positions persist into the existing additive `visualLayout`).
- **Shared-file edits (additive):** `index.html` toolbar (2 buttons), `domUtils.js` (2 refs), `uiUtils.js` `updateViewToggle` (2 show/hide lines), `eventHandlers.js` (button wiring + `Cmd/Ctrl+Z` shortcut + `handleTidyUp`/`handleUndoTidyUp`/`handleJumpToNextError`), `styles.css` (`/* === WAVE2 LANE canvas === */` block). No new JS module → no new `build.files` entry (`autoLayout.js`/`eventHandlers.js`/`flowVisualizer.js` already listed).
- **Tests:** `__tests__/flowVisualizerLaneCanvas.test.js` — 14 TDD cases (apply mapping, stale-id tolerance, Tidy-selection, idempotency, undo, relayout-animation class, badge state machine, highlight/runtime-info wiring, jump cycling, badge survives re-render). Suite: 253 → **267 passing**.
