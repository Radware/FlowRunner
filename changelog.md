...
## Unreleased — Command/search palette + read-only View-as-JSON (UX Wave 2, lane node-features)
- **Decision:** shipped two **overlay** modules that never touch `flowVisualizer.js` (respecting the visualizer contract) and hold no app state — they consume callbacks passed in per-open.
  - **`palette.js`** — a reusable command/search palette. Two consumers, one component: (i) **add-node search** (Tab, or double-click empty visualizer canvas) lists the four step types and, on select, calls straight into the existing add-step plumbing (`insertStepAfterInModel` when a step is selected, else the central `handleBuilderStepUpdate` 'add'); (ii) a global **Cmd/Ctrl+K command palette** for actions + navigation + open-recent, built from live app state so it never offers a disabled/no-op command. **Why fuzzy in-house, not fuse.js:** a ~40-line subsequence scorer keeps the CSP `script-src 'self'` story clean (no vendored lib to plumb) and the ranking is unit-testable; the public surface (`fuzzyFilter`) is fuse-shaped so swapping fuse in later is localized. Fuzzy core (`fuzzyMatch`/`fuzzyFilter`) is pure and separately tested from the DOM overlay.
  - **`jsonView.js`** — a **read-only** "View as JSON" panel showing the flow's canonical `.flow.json` with a **line diff against the last-saved bytes**. Serialization goes through the SAME `flowCore.flowModelToJson` the save path uses, so the panel shows exactly what would be written and the diff is true. The last-saved baseline is read from disk and re-normalized through `jsonToFlowModel → flowModelToJson`, so cosmetic formatting never shows as a diff; a never-saved flow reads as fully added. **Editing is deferred** (no textarea/contenteditable) until `.flow.json` round-trip safety — marker-encoded bodies, extract-path normalization — is proven, since this file is shared with flowrunner-cli + the portal.
- **Wiring (additive):** `eventHandlers.js` gained `initializePaletteShortcuts()` (Cmd/Ctrl+K, Tab, dblclick-empty-canvas) called from `initializeEventListeners()`, and the "Toggle View as JSON" command. `index.html` gained two mount points (`#palette-host`, `#json-view-mount`). `styles.css` got a delimited `/* === WAVE2 LANE node-features === */` block consuming only OKLCH semantic tokens (no hex, no `#000/#fff`).
- **Deps + build.files:** no new runtime dependency (fuse.js avoided). Added `palette.js` and `jsonView.js` to `build.files` per the #1 rule.
- **Tests (TDD):** `__tests__/palette.test.js` + `__tests__/jsonView.test.js` — 42 cases covering the pure fuzzy scorer/ranker, the pure LCS line-diff + canonical serialization, and both DOM overlays (open/filter/keyboard-nav/select/destroy; read-only guarantee; diff add/remove rows; raw-mode toggle). Full suite green.
