...
## Unreleased — Fuzzy search + undo/redo (UX Wave 2, lane file-features)
- **Decision:** added **Fuse.js** fuzzy search over the recent-files sidebar list AND the current flow's steps, plus **immer-patch** undo/redo over flow-model edits (Cmd/Ctrl+Z, Cmd/Ctrl+Shift+Z / Ctrl+Y). Both libraries are **vendored as single-file ESM** under `assets/vendor/{fuse,immer}/…mjs` and imported by **relative path**, not bare specifier. **Why:** the renderer CSP is `script-src 'self'` with no Node integration — a bare `import Fuse from 'fuse.js'` does not resolve in the browser context, and vendoring the `.mjs` (like Drawflow) is the only CSP-clean path. The npm packages stay in `dependencies` purely so Jest (node_modules resolver) can run the same code; the packaged app never bundles them.
- **New pure modules (unit-tested under jsdom):** `fuzzySearch.js` (`searchFiles`/`searchSteps`/`filterList`, flattens then/else/loop nesting so nested steps are searchable) and `undoManager.js` (a zundo-style history: `produceWithPatches` records forward+inverse patches; undo/redo apply them; no-op patch sets are ignored; depth-capped via `config.js` `UNDO_HISTORY_LIMIT`). `flowHistory.js` is the app glue binding the pure history to `appState.currentFlowModel`.
- **Undo/dirty composition:** `flowHistory` auto-detects a *new flow* by **model object identity** in `renderCurrentFlow` (loads/creates/clones reassign the reference; in-place edits mutate it), so no reset call had to be threaded through every file-lifecycle handler. It reconciles `appState.isDirty` against the last **saved** baseline (`markFlowHistorySaved()` on save success), so undo/redo back to a saved state correctly reports clean and drives Save/Cancel/Close enablement through the existing `setDirty()`.
- **Search is non-destructive:** filters toggle a `.search-hidden` class on already-rendered rows (and reorders file rows by relevance) instead of re-rendering, preserving drag order, selection, and any open step editors. TDD: `__tests__/{fuzzySearch,undoManager,flowHistory}.test.js` (+41 tests, 253→294). Added the three JS modules to `build.files` (vendored `.mjs` are covered by the existing `assets/**/*`). Deps: `fuse.js`, `immer`.
