...
> **⚠️ This format is a CROSS-APP CONTRACT**, not a private file. Three apps read/write `.flow.json`: FlowRunner UI (this repo, JS), **flowrunner-cli** (Python — headless, runs flows 24/7, *stricter* parser), and the **ShowRunner Demo-Management-Portal** (Python + React orchestrator). Rules: **additive-only** (new fields optional; the CLI ignores unknown fields but *drops* them on round-trip — no app is a pass-through); **never rename/repurpose** the frozen fields (`staticVars`; the wire keys `then`/`else`/`steps` — never the internal `thenSteps`/`elseSteps`/`loopSteps`; `onFailure`, required by the CLI; `type` ∈ {request,condition,loop,transform}, which the CLI *rejects* if unknown; the `##VAR:type:name##` body markers; the `conditionData` operator vocabulary; extract namespaces; the method allow-list). New step types / operators must ship in the CLI **before** any flow uses them. There is **no `schemaVersion`** field yet — the planned evolution adds one. See [gotchas.md](gotchas.md) (Cross-app FlowMap contract) and [masterplan.md](masterplan.md) (Ecosystem), and the full plan in [docs/flowmap-evolution.md](docs/flowmap-evolution.md).
...
- **CI** (`.github/workflows/build.yml`): macOS + Windows + Ubuntu matrix. Triggers — **push** to `main`/`master` (build **+ publish**), **pull_request** and **workflow_dispatch** (build only, no publish); all honor `paths-ignore` (`**.md`, `docs/**`, `.vscode/**`, `schemas/**`). The `release` job is gated to push-on-`main`/`master` and creates/updates the GitHub Release tagged `v{VERSION}` from `release.md`. Build a branch without publishing: `gh workflow run "FlowRunner Build" --ref <branch>` → `gh run download <run-id>`.
